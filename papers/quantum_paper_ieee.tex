...
\subsection{Sparse-by-construction observables: a complementary reduction}\label{sec:sparse}

Measurement grouping takes the observables as given and reduces the cost of measuring them. We also tested building the observables sparse from the start -- nonzero only within their own feature block -- instead of dense matrices grouped after the fact. On the $K{=}3$ raw-feature construction (5 qubits, $d{=}21$), this cuts the raw Pauli-term count a further \textbf{2.4$\times$} (4752$\to$1936 for the nine quantities) and the qubit-wise-commuting count a further \textbf{3$\times$} (1098$\to$366). At the larger scale actually used by the residual-coupled fix (Section~\ref{sec:v3}, 99 trees, 7 qubits), the reduction is similar: 74{,}304$\to$24{,}960 raw terms, 9846$\to$2676 after qubit-wise grouping, a \textbf{3.7$\times$} reduction on top of grouping.

This has a real, if small, accuracy cost. On the original formalism, sparse observables give $R^2=0.489\pm0.045$ on band gap versus $0.513\pm0.046$ for dense observables (paired $t=-3.27$, $p=0.031$, 5 trials) -- a small but statistically significant drop. The mechanism is structural, not incidental: two observables with non-overlapping feature blocks have an \emph{exactly} zero symmetrized covariance under this construction, so sparsity removes real cross-observable information along with the measurement overhead. On the residual-coupled fix, where the tree-group blocks are non-overlapping by construction, this effect is total: every pairwise covariance is exactly zero (confirmed to machine precision at both the 21- and 99-dimensional scale), so the covariance term contributes nothing by construction rather than by empirical accident there. The resulting accuracy difference is smaller and not significant ($R^2=0.556\pm0.030$ versus $0.590\pm0.066$, $p=0.302$). Sparse-by-construction observables are a genuine additional lever for cutting NISQ cost, but they sharpen rather than change the paper's central finding: whatever the covariance term contributes is not large enough to survive either measurement-efficient design choice. Full numbers in \texttt{results/sparse\_observable\_experiment.json}.
...
The formalism admits an exact, verified quantum circuit realization -- this is a genuine (if narrow) confirmation that the mathematics in Section~\ref{sec:method} is not just an analogy. The realization is expensive in exactly the way the formalism's own structure predicts (dense, non-sparse observables; combinatorial growth in $K$; shot budgets further degraded by realistic gate noise), but standard measurement-grouping techniques provide a genuine, substantial ($4$--$12\times$) reduction in that cost, and building the observables sparse from the start provides a further $2$--$4\times$ reduction on top of grouping, at a small, real accuracy cost. None of this bears on whether the formalism helps active learning (Sections~\ref{sec:results}--\ref{sec:v3} already answer that, mostly negatively); it characterizes what it would cost, and how that cost can be partially mitigated, to run the classically-simulated experiments above on an actual quantum device -- for a construction that the classical simulation already showed provides no accuracy benefit over simply measuring ensemble disagreement.
