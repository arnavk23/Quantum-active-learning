...
Measurement grouping treats the observables as given and reduces the cost of measuring them. We also tested the complementary approach: building the observables sparse from the start, nonzero only within their own feature block, rather than dense matrices grouped after the fact. On the $K=3$ raw-feature construction (5 qubits), this cuts the raw Pauli-term count a further 2.4-fold (4,752 to 1,936 for the nine quantities) and the qubit-wise-commuting count a further 3-fold (1,098 to 366; Figure~\ref{fig:sparse}). At the larger, practically relevant scale used by the residual-coupled fix (99 trees, 7 qubits), the reduction is similar: 74,304 to 24,960 raw terms, 9,846 to 2,676 after qubit-wise grouping, a 3.7-fold reduction on top of grouping.
...
Sparsity also has a real, if small, cost. On the original formalism, sparse observables give $R^2=0.489\pm0.045$ on band gap versus $0.513\pm0.046$ for dense observables, a small but statistically significant drop (paired $t=-3.27$, $p=0.031$, 5 trials). The reason is structural: two observables with non-overlapping feature blocks have an exactly zero symmetrized covariance under this construction, not just a small one, so sparsity removes real cross-observable information along with the measurement overhead. On the residual-coupled fix (Section~\ref{sec:v3}), where the observable blocks are non-overlapping tree groups by construction, this effect is total rather than partial: every pairwise covariance is exactly zero (confirmed to machine precision at both the 21- and 99-dimensional scale, not merely small), so the covariance term contributes nothing by construction rather than by empirical accident. The accuracy difference there is correspondingly smaller and not significant ($R^2=0.556\pm0.030$ versus $0.590\pm0.066$, $p=0.302$). Sparse-by-construction observables are a genuine additional lever for reducing NISQ cost, but they sharpen rather than change the paper's central finding: whatever the covariance term contributes, it is not large enough to survive either measurement-efficient engineering choice.
