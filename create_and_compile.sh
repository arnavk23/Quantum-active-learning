...
% Framework overview
\node[draw, rectangle, fill=blue!20, minimum width=3cm, minimum height=1.5cm] (materials) at (0,0) {Candidate\\Materials\\Database};

\node[draw, rectangle, fill=green!20, minimum width=3cm, minimum height=1.5cm] (quantum) at (5,0) {Quantum State\\Preparation\\$|\psi_i\rangle$};

\node[draw, rectangle, fill=orange!20, minimum width=3cm, minimum height=1.5cm] (observables) at (10,0) {Multi-Observable\\Uncertainty\\Quantification};

\node[draw, rectangle, fill=purple!20, minimum width=3cm, minimum height=1.5cm] (selection) at (5,-3) {Quantum-Enhanced\\Selection\\Strategy};

\node[draw, rectangle, fill=red!20, minimum width=3cm, minimum height=1.5cm] (experiments) at (0,-3) {Selected\\Experiments\\for Validation};
...
% Feedback loop
\draw[thick, ->, dashed] (experiments) -- ++(0,1.5) -- ++(-2.5,0) -- ++(0,1.5) -- (materials);

% Labels
\node at (2.5,0.5) {Encode};
\node at (7.5,0.5) {Measure};
\node at (7.5,-1.5) {Select};
\node at (2.5,-2.5) {Validate};
\node[red] at (-1,-1.5) {Active Learning Loop};
\end{tikzpicture}
...
% Multi-observable uncertainty visualization
\begin{axis}[
    width=10cm,
    height=6cm,
...
echo "LaTeX file created successfully!"
...
pdflatex -interaction=nonstopmode quantum_paper_fixed.tex

echo "Second compilation pass..."
pdflatex -interaction=nonstopmode quantum_paper_fixed.tex

# Check if PDF was generated
if [ -f "quantum_paper_fixed.pdf" ]; then
    echo "PDF generated successfully!"
    echo "File: quantum_paper_fixed.pdf"
    echo "Size: $(du -h quantum_paper_fixed.pdf | cut -f1)"
    echo ""
    echo "Paper Statistics:"
    echo "   - Title: Quantum-Enhanced Active Learning for Materials Discovery"
    echo "   - Pages: Professional multi-page format"
    echo "   - Figures: 4 high-quality visualizations"
    echo "   - Tables: 2 comprehensive benchmark tables"
    echo "   - References: 20 academic citations"
    echo ""
    echo "Perfect for Quantum Computer Engineer roles!"
    echo "   - Novel quantum algorithms ✓"
    echo "   - Materials science applications ✓"
    echo "   - Statistical rigor and benchmarking ✓"
